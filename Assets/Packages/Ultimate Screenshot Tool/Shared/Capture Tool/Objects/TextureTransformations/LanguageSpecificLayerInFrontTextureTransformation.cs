...
{
    [System.Serializable]
    public class LanguageSpecificLayerInFrontTextureTransformation : LayerInFrontTextureTransformation
    {
        ...
        {
            return "Language Specific Layer In Front";
        }
        ...
        public override Texture2D ApplyTransformation(Texture2D texture, bool apply = true, bool destroyOriginal = true)
        {
            ...
            for (int i = 0; i < otherLayerForLanguage.Length; ++i)
            {
                if(Application.systemLanguage == otherLayerForLanguage[i].language)
                    foreground = otherLayerForLanguage[i].texture;
            }
        ...
        {
            LanguageSpecificLayerInFrontTextureTransformation clone = ScriptableObject.CreateInstance<LanguageSpecificLayerInFrontTextureTransformation>();
