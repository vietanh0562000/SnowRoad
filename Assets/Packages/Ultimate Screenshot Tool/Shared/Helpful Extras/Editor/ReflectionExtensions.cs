...
namespace TRS.CaptureTool.Extras
{
    ...
    {
        public static Type[] GetSubclasses(this Type classType)
        {
            return new List<Type>(classType.Assembly.GetTypes().Where(type => type.IsSubclassOf(classType))).ToArray();
        }

        public static string[] GetSubclassNames(this Type classType)
        {
            return GetSubclasses(classType).Select(x => x.Name).ToArray();
        }
