	...
	SubShader
	{
		...
		Pass
		{
			...
			) : SV_Target
			{
				...
				float __layer_snow = saturate(  ( input.pack0.xyz.y * _PositionRange_snow ) + _PositionThreshold_snow );
		...
		HLSLINCLUDE
			...
			) : SV_TARGET
			{
				...
				float __layer_snow = saturate(  ( input.pack0.xyz.y * _PositionRange_snow ) + _PositionThreshold_snow );
				float __contrast_snow = ( _contrast_snow );
				float __noise_snow = (  saturate( TCP2_TEX2D_SAMPLE(_NoiseTexture_snow, _NoiseTexture_snow, input.pack1.xy * _NoiseTexture_snow_ST.xy + _NoiseTexture_snow_ST.zw).r * imp_101 ) - imp_101 / 2.0 );
