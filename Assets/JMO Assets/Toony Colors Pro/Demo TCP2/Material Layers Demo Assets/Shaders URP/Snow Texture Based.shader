...
Shader "Toony Colors Pro 2/Examples/Material Layers/Snow (Texture Based)"
{
	...
	SubShader
	{
		...
		TCP2_TEX2D_WITH_SAMPLER(_layer_8f0527);
		...
		Pass
		{
			...
			) : SV_Target
			{
				...
				float __layer_snow = saturate( TCP2_TEX2D_SAMPLE(_layer_8f0527, _layer_8f0527, input.pack2.xy * _layer_8f0527_ST.xy + _layer_8f0527_ST.zw).r );
		...
		HLSLINCLUDE
			...
			) : SV_TARGET
			{
				...
				float4 __mainColor = ( _BaseColor.rgba );
				float __alpha = ( __albedo.a * __mainColor.a );
				float __layer_snow = saturate( TCP2_TEX2D_SAMPLE(_layer_8f0527, _layer_8f0527, input.pack0.xy * _layer_8f0527_ST.xy + _layer_8f0527_ST.zw).r );
				float __contrast_snow = ( _contrast_snow );
