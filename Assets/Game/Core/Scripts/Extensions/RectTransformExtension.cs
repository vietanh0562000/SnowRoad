...
namespace BasePuzzle
{
    ...
    public static class RectTransformExtension
    {
        ...
        /// <summary>
        /// Determines if this Rect have position inside the screens space view
        /// </summary>
        public static bool IsPositionVisible(this RectTransform rectTransform, Camera camera = null)
        {
            if (!rectTransform.gameObject.activeInHierarchy) return false;
            
            Rect    screenBounds          = new Rect(0f, 0f, Screen.width, Screen.height);
            Vector3 tempScreenSpaceCorner = rectTransform.position; // Cached
            if (camera != null)
            {
                // Transform world space position of corner to screen space
                tempScreenSpaceCorner = camera.WorldToScreenPoint(rectTransform.position); 
            }
            
            return screenBounds.Contains(tempScreenSpaceCorner);
        }
    }
}